...
export class CreacionUsuarioUseCase {
  ...
  async execute(
    ...
    const fechaNacimiento = new FechaNacimiento(dto.fechaNacimiento);
    ...
    const usuario = Usuario.crearEstudiante(correo, hash, idConsentimiento);
    ...
    await this.usuarioRepository.crear(usuario, lineaBase, session.sesion);

    return {
      tokenSesion: session.tokenSesion,
      csrfToken: session.csrfToken,
      respuesta: CreacionUsuarioDtoResponse.crear(usuario, lineaBase),
    };
  }
}
