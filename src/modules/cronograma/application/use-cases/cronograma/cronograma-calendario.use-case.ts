...
export class CronogramaCalendarioUseCase {
    ...
    async execute(request: CronogramaCalendarioDtoRequest): Promise<ObtenerContenidosUnidadDtoResponse> {

        const cronograma = await this.cronogramaRepository.buscarPorId(request.idCronograma);
        if (!cronograma) {
            throw new CronogramaNoEncontradoException();
        }

        const unidadTemporal = await this.unidadTemporalRepository.obtenerPorIdUnidadTemporal(request.idUnidadTemporal);
        if (!unidadTemporal) {
        ...
        const relacionesDB = await this.contenidoCronogramaRepository.obtenerContenidosConRecursosPorUnidadTemporal(request.idUnidadTemporal);
